...
int send_msg(int msg) {
    ...
    if (pid != NULL) {
        ...
        struct task_struct *cuxxx = pid_task(pid, PIDTYPE_PID);

        if (cuxxx != NULL) {


            struct kernel_siginfo *info = (struct kernel_siginfo  *)kzalloc(sizeof(struct kernel_siginfo ),GFP_KERNEL);

            int ret;
            info->si_signo = SIGUSR1;
            info->si_code = SI_QUEUE;
            info->si_int = 123;
             ret = send_sig_info(SIGINT,info, cuxxx);
             kfree(info);
             if(ret < 0)
            printk("point not null\n");
        } else {
            printk("point null\n");
        }
    } else {
